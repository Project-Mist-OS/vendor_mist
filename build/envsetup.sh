...
function breakfast()
{
    target=$1
    local variant=$2
    local aosp_target_release=$(cat ${ANDROID_BUILD_TOP}/vendor/mist/vars/aosp_target_release 2>/dev/null)
...
function mistrebase() {
    ...
    git fetch "http://review.mistos.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
