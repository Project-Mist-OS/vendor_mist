...
MIST_TARGET_PACKAGE := $(PRODUCT_OUT)/MistOS-$(MIST_VERSION).zip
...
bacon: $(DEFAULT_GOAL) $(INTERNAL_OTA_PACKAGE_TARGET)
	...
	echo -e ${CL_BLD}${CL_GRN}"Size:"${CL_RED}" `du -sh $(MIST_TARGET_PACKAGE) | awk '{print $$1}' `"${CL_RST}
	echo -e ${CL_BLD}${CL_GRN}"ID:"${CL_RED}" `md5sum $(MIST_TARGET_PACKAGE) | cut -d ' ' -f 1`"${CL_RST}
	echo -e ${CL_BLD}${CL_GRN}"Path:"${CL_RED}" $(MIST_TARGET_PACKAGE)"${CL_RST}
