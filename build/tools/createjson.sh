...
existingOTAjson=./vendor/official_devices/$1.json
output=$2/$1.json
#cleanup old file
if [ -f $output ]; then
	rm $output
...
if [ -f $existingOTAjson ]; then
	...
	"response": [
		{
			...
			"device": "'$device'",
			"filename": "'$filename'",
			"download": "https://sourceforge.net/projects/project-mistos/files/Android14/${DEVICE}/${file_name}/download",
			"timestamp": '$timestamp',
			"md5": "'$md5'",
			"sha256": "'$sha256'",
			"size": '$size',
	...
	echo 'Consider adding official support'
fi
echo ""
