...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:MistOS-Devices+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the MistOS snippet
    try:
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        repo_remote = repository.get("remote", "mist-devices")
        repo_revision = repository.get('revision')

        if repo_remote != "mist-devices" and not repo_remote.startswith("aosp-"):
        ...
        if repo_revision and not (repo_remote == "mist-devices" and repo_revision == get_default_or_fallback_revision(repo_name)):
...
def get_default_or_fallback_revision(repo_name):
    default_revision = get_remote_revision_from_snippet(".repo/manifests/snippets/mist.xml", "mist-devices")
...
print("Repository for %s not found in the MistOS-Devices Github repository list. If this is an error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
