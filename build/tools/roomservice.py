...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from MistOS Github (http://github.com/Project-Mist-OS)." % device)
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('MistOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: MistOS/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "MistOS/%s" % repo_name })
