...
if [ "$1" ]; then
    ...
    DEVICE=$(echo $TARGET_PRODUCT | sed 's/mist_//g')
    if [ -f $file_path ]; then
        ...
        if [[ $file_name == *"OFFICIAL"* ]] || [[ $FORCE_JSON == 1 ]]; then
            ...
            build_type=$(grep ro\.mist\.build\.type ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            base_version=$(grep ro\.mist\.base\.version ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            link="https://sourceforge.net/projects/project-mistos/files/${DEVICE}/${file_name}/download"
            echo "{" > $file_path.json
